...
const { getTimestamp } = require('../../helpers/prepareRead');
...
router.get('/', authenticateToken, async (req, res, next) => {
  ...
  try {
    const data = await pool.query(
      'SELECT id, start_date AS "startDate", end_date AS "endDate", rating, notes FROM reads WHERE book_id = $1 ORDER BY end_date DESC;',
      [bookId]
    );
