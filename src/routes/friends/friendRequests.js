...
router.get('/', authenticateToken, async (req, res, next) => {
  ...
  try {
    const data = await pool.query(
      'SELECT FR.id AS "requestId", FR.date as "requestDate", users.id AS "senderId", users.first_name AS "firstName", users.last_name AS "lastName", users.profile_image AS "profileImage" FROM friend_requests FR INNER JOIN users ON FR.sender_id = users.Id WHERE FR.receiver_id = $1 ORDER BY "requestDate" DESC',
      [user.id]
    );
...
// Answer (accept / decline) a friend requests
